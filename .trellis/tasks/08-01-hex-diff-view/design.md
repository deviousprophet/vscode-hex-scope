...
- **Selection + copy (D29).** Click-drag on a `[data-side][data-addr]` cell selects a byte range locked to one side; selection-clear only on empty `#diff-scroll` clicks. Ctrl+C → `formatCopyCommand` (hex) over `selectionBytes()` (present bytes in address order).
