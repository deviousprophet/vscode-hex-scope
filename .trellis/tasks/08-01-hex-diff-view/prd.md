...
D30. **Explorer menu switch (single vs double select).** In the explorer context menu:
    - **Compare Two Files** is a **direct `explorer/context` item** gated by `resourceLangId && listDoubleSelection` — direct items reliably receive the `Uri[]` selection on multi-select (submenu commands do not, which made it a silent no-op).
    - The **HexScope submenu** (Open, Quick Repair, Set as 1st file, Compare with the 1st file) is gated by `resourceLangId && !listMultiSelection` — hidden on multi-select since its items are single-file actions. Inside, staging items keep `!listMultiSelection` guards.
    So single-select shows only the staging flow; exactly-2 shows only Compare Two Files; >2 shows neither. The submenu's `when` makes staging items appear in the editor-title menus (single active editor, no list selection) while Compare Two Files stays explorer-only. The staged state is **cleared as soon as a diff opens** (either entry path), so the badge and "Compare with the 1st file" item disappear immediately (D7). Staging is **menu-only** — no staging keybindings are contributed (a keyboard chord on a menu item was noise); bare `Alt+↓/↑` stays reserved for in-diff navigation (D20).
