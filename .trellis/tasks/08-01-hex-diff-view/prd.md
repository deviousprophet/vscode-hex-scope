...
D24. **Visual parity (R8).** The diff webview loads the shared `base.css` design tokens and mirrors the editor's cell / address / toolbar / summary-bar conventions rather than defining its own look. **Diff statuses collapse to one "diff" look**: changed / added / removed all render as red (a byte either differs or it doesn't — the add/remove distinction is dropped in the diff view). Interaction states are **independent visual channels** so every combination stays legible: status = background+text, selection = cyan outline, cross-panel selection = dashed cyan outline, hover = inner ring, search match = amber underline (current match thicker), column hover = faint background on non-status cells. The webview HTML must include every DOM node the view script references at bootstrap (no `getElementById(...)!` on missing nodes) and a CSP that permits the renderer's inline geometry styles, or the renderer must not rely on them. Rationale: users already read the hex-grid grammar; a divergent look would misread as a different tool.
...
D36. **Byte-cell design (approved final, spec `frontend/hex-view-component.md`).** A byte cell = a base class (`bn` normal / `bz` zero / `bp` print / `bh` highlight / `be` empty / `bd` diff) + combo classes (hover / column / selection / match). All differences collapse to `bd` (red text; dim red bg `rgba(255,90,90,.10)` only on base and hover `rgba(255,90,90,.16)`). **`bd` uses `bn`'s background for every state except base/hover** — sel = `--sel-bg` fill, match = faded yellow `rgba(234,179,8,.22)`, sel+match = yellow + blue border — each with **red text** (the diff signal). No `amatch` state; the current search match is shown by the row `search-row` highlight. `be` cells: no hover/sel/match/edit (N/A). `bd`: no dirty/editing. This design is also the basis for the future single-view reuse of the component.
