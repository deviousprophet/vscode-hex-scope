...
- Wide bitunit tree connector: the `.si-arr-grp-hdr::before` / `.si-arr-el-hdr::before` vertical tree dash (`top: 50%`) centers over a wrapped header and floats near the second line. Wide bitunit headers (>2 storage bytes, i.e. u32/u64) get a `.si-bitunit-wide` class; its `::before` re-anchors to `bottom` so the dash sits next to the chevron on the header's last line.
...
- CSS in `src/webview/components/sidebar/structPanel/structPanel.css`; one marker class (`si-bitunit-wide`) added from `src/webview/components/sidebar/structPanel/structPanel.ts`.
...
- [ ] Manual: u32/u64 bit-field parent shown in binary — `>` chevron, field name, endian chip, dotted leader all sit on the binary value's **last** line; the dotted leader is on the same baseline as the name/bit text (no drop below it); the tree connector dash sits on that same last line next to the chevron (not floating mid-row).
