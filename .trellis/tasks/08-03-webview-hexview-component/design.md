...
- **Single-view ASCII toggle (small feature):** host adds a toggle in the memory view toolbar that flips the render input `showAscii` between `true` and `false`. Default `true` (byte-identical to current single view). Toggle labels: **Hex** / **Hex+ASCII**. Component stays presentational — it only honors the `showAscii` input; host owns the button + state.
