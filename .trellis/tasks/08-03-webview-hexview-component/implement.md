...
4. **Rewrite host wiring** `src/webview/hexViewer.ts`
   ...
   - Add memory-view ASCII toggle: button/small control in toolbar flipping render input `showAscii` between `true` and `false` (default `true`); labels **Show ASCII** with active/pressed state. Host owns the toggle state; component only honors the input.
