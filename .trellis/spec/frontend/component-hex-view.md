...
- **Root-scoped (diff-compatible):** constructor `rootSelector`, no global DOM ids; single instance today, two panels later by passing selectors.
...
- ASCII toggle (small feature): toolbar button flips `showAscii` (default true); labels **Hex** / **Hex+ASCII**; host owns state, component honors input.
