...
Applies to memory selection/drag behavior, Inspector, context commands/menu, and `core/byte-tools/`. Search engine behavior is owned by [Search Engine Code-Spec](./search-engine.md); search bar UI by [SearchBar Component Code-Spec](./search-bar-component.md).
