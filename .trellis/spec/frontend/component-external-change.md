# Component Spec ΓÇö ExternalChange
...
Boundary rule (issue #151 AC): the component owns banner markup, per-banner dismiss wiring, and styles. It never reads/writes `S`, never reloads/repairs files, never mutates `IncomingFile` ΓÇö it renders and reports.
...
- **Dismiss parity:** conflict/reload buttons ΓåÆ `banner.remove()` then host `onReload(incoming)`; error buttons ΓåÆ host `onRepair()`/`onViewText()` only (host reload flow removes the banner).
- **Host never writes banner DOM** ΓÇö uses `clearAll()`/`clearError()`.
- **Lock-state is NOT in this component** ΓÇö `src/webview/lock.ts` (`updateExternalChangeLockState` + disable/enable of `button/input/[role=button]` in `#main-area`/`#toolbar`, `data-was-enabled` round-trip). Host owns lock-state transitions.
...
- Conflict: `#ext-conflict-banner.ext-conflict-banner` ΓÇö ΓÜá icon, "File changed externally. You have N unsaved edit(s). Changes must be reloaded.", "Reload & discard my edits" button ΓåÆ remove + `onReload`.
- Reload: `#ext-reload-banner.ext-reload-banner` ΓÇö ≡ƒöä icon, "File changed externally. Reloading...", "Reload" button ΓåÆ remove + `onReload`.
- Error: `#ext-error-banner.ext-error-banner` ΓÇö Γ¥î icon, "File changed externally and is now invalid: N checksum error(s) and/or N malformed line(s)", "Quick Repair & reload" (`canQuickRepair`) else "View in text editor" ΓåÆ `onRepair`/`onViewText`.
