...
- Fast save: `spliceEditedLines(raw, edits, format)` scans lines with a minimal regex decode (no `HexRecord` materialize) and rebuilds only data records whose absolute address range overlaps the edit set (checksum recomputed). Used by the `saveEdits` path so a few edits on a 70 MB file cost O(edits + split/join + one write), not a full serialize/reparse.
