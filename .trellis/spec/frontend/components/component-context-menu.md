...
- **Keyboard/ARIA (WAI-ARIA menu pattern):** `#ctx-menu` has `role="menu"`; rows are `role="menuitem" tabindex="-1"` (disabled rows `aria-disabled`), separators `role="separator"`. `show()` focuses the first enabled row; ArrowUp/ArrowDown move focus among enabled rows **scoped strictly within the open submenu** (wrap at the ends; when no submenu is open, only the parent menu's own rows — submenu triggers remain on the parent list); ArrowRight on a `.ctx-has-sub` row opens its submenu and focuses the first enabled item inside; ArrowLeft inside an open submenu closes it and returns focus to the parent row; Enter/Space activates (or opens the submenu on a `.ctx-has-sub` row); Escape closes the open submenu first, then the whole menu on the next press (no submenu → first press hides). The host opens the menu on the grid `ContextMenu` key / Shift+F10. `show()` records the previously focused element; `hide()` restores focus to it (when still connected) so keyboard control returns to the triggering grid/sidebar row instead of the body. **Input-modality highlight:** `#ctx-menu.ctx-kb` gates the `:focus-visible` selection highlight — any `keydown` adds `.ctx-kb`, any `pointerdown` removes it, `show()` sets it from the tracked last input mode. Result: mouse-right-click opens with no first-row highlight (hover only); the ContextMenu key / Shift+F10 or any later keypress lights the keyboard selection and arrows navigate from the first row either way. Submenu open-state checks use `style.display === 'block'` (CSS default `''` is closed).
...
| Click outside / Escape | Menu hides (Escape is two-step when a submenu is open). |
| Focus leaves menu / window blur | Menu hides (`focusout` + `blur`; inside-menu focus moves keep it open). |
| ArrowRight on submenu row | Submenu opens, first enabled item focused. |
| ArrowLeft inside open submenu | Submenu closes, parent row refocused. |
