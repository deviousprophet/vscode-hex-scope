...
- Markup is byte-identical to pre-refactor (same ids/classes: `#s-scripts`, `scripts-count`, `scripts-refresh`, `.script-toolbar`, `.script-card`, `.script-run-btn`, `.script-result-area`, `.script-output-block`/`-hdr`/`-log`, `.script-cap`, `.script-ext`, `.script-dot`). All CSS moved verbatim from `styles/sidebar.css`. Untrusted text escaped with `esc()`; CSS-attribute paths escaped with `cssEscape` (Windows backslashes — scripting.md §9.1).
...
- Pure helpers (`cssEscape`, `extLabel`, `capBadges`, `btnTitle`/`btnClass`/`scriptBtnAttrs`, `writesBlockHtml`) stay module-level and DOM-free.
...
| Second script clicked while running | Ignored (no run, no cancel) |
...
| Re-run | Prior result block replaced |
