...
- **Loading card + progress:** `diffHtml` renders the shared `.loading-*` card (`#diff-loading`) in `#app`; `DiffEditorPanel` reads and parses both files concurrently (`Promise.all`) and posts throttled `diffProgress` (`read`/`parse`/`diff`, completed/total) by summing the two per-file fractions (`src/diff/loadProgress.ts`; read fills `0 → 0.5`, parse `0.5 → 1`, `total = 2`) while diffing. The card is swapped for `#diff-root` on `diffInit`/`diffError` (`applyDiffProgress` updates text + bar fill meanwhile).
