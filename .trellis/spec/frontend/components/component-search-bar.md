...
- Run button triggers `onSearch(..., 'button')`; prev/next navigate; clear empties input + `onClear`. Prev/next buttons are `#btn-prev`/`#btn-next` with glyphs `▲`/`▼` (an earlier polish iteration used `◀`/`▶`; `▲`/`▼` is the intended shipped form).
