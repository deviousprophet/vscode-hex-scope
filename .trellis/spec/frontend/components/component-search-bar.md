...
- **Second surface (diff view):** `src/webview/diff/diffSearch.ts` reuses `SearchBar` inside the isolated diff bundle (`import '../components/searchBar/searchBar'` + `searchBar.css`). There it is always visible — mounted on boot into the toolbar's row-1 right slot and re-injected by `mountDiffSearch()` after each toolbar re-render; there is no `Find` toggle/open-close gating (the host keeps no `findVisible` state) and `Ctrl+F` focus/select stays in the component. The component stays host-agnostic there too — it never reads `S`, and the diff host owns execution: one query over both sides' hydrated segments, unioned/deduped addresses, painting via the grids' render input, and `scrollToDiff` for navigation. Because both surfaces are separate bundles/realms, the global ids do not collide. Full parity with the hex host: the diff host also runs repeat-Enter navigation on an unchanged completed query, streamed progress (paint + count + one-time first jump), active-match selection, modulo next/prev wrap, divergence-gated clearing, and count refresh after a toolbar re-render. Both hosts share the pure decisions in `src/webview/search/searchNavigation.ts` (`shouldNavigateCompletedSearch`, `isSearchDiverged`) — importing `searchEngine.ts` from the diff bundle would break isolation because it reads `S`/`memoryGrid` (see `component-diff-view.md`).
