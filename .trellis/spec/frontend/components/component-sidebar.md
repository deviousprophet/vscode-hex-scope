...
- Sash UX: drag resizes the pane above (clamped so every pane keeps min = header 22px + 60px); a drag delta is applied to the pane's **displayed** px (so a drag right after a proportional fill does not jump back to the stale saved px); ArrowUp/ArrowDown resize by ±10px; double-click resets the two adjacent panes' combined space 50/50. Sashes adjacent to a collapsed pane get `.disabled` (inert; out of the tab order, `aria-disabled`). Sash drags persist the adjacent sizes continuously during the drag; arrow-resize and dblclick persist immediately. A sash click with no movement persists nothing and does not mark panes user-set (they stay fresh/auto-default).
...
- Persistence: `hexScope.sidebarPanes.<panelId>.<sectionId>` stores the pane's last expanded px (collapse keeps it so re-expand restores). Restore parses, clamps to the valid minimum, and drops NaN/≤0 entries (removing the key); oversized values clamp to the available pool on the first layout. The allocated sizes always sum exactly to the pool: all-user panes that under-fill scale up proportionally (rounding remainder on the last pane). First-time default shares (50/50 among expanded) are not persisted until the user drags or resizes; a no-movement sash click writes nothing.
