...
- Collapse state is per mounted instance (map), survives body re-renders, resets when the panel shell is rebuilt. Collapse keeps the pane in DOM order and animates `flex-basis` to 22px (150ms ease-out, VS Code duration; `prefers-reduced-motion` → 0s); the body stays in the DOM (clipped by the 22px pane). Expand restores the saved px (or an equal share of the free space the first time); siblings shrink proportionally. No grid rows, no reparent, no timers, no disabled-sash state.
