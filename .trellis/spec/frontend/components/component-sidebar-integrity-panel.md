...
- Markup is byte-identical to pre-refactor (same ids/classes: `#s-integrity`, `integrity-shell`, `integrity-profiles`, `integrity-card`, `integrity-check-form`, `data-check-id`, `data-draft-control`, `data-form-action`, `data-check-toggle`, `data-auto-fix`, `data-copy-calculated`, `data-check-status`); all CSS moved verbatim from `styles/integrity.css`. Untrusted text escaped with `esc()`.
