...
- Webview sends `ready`; host reads and parses both files **concurrently** (`Promise.all`, no serial 2× parse penalty) and streams throttled `diffProgress` (`read` → `parse` → `diff`), then responds with one `diffInit` (generation-stamped) or `diffError`. Each file contributes a fraction in `[0,1]` — its read fills the first half (`0 → 0.5`, `READ_SHARE`) and its parse the second (`0.5 → 1`, relayed from the parser's `onProgress`) — so `parse` never regresses below the read fraction. `completed = combinedLoadProgress([fractionA, fractionB])` (`src/diff/loadProgress.ts`, clamped sum) with `total = 2`: monotonic even when the two files' reports interleave. Stage is `read` while any file is still ≤ `0.5`, else `parse`; `diff` brackets `computeByteDiff` (`0 → 1`).
...
- `src/test/core/diffLoadProgress.test.ts`: `combinedLoadProgress` sums the two per-file fractions, clamps an out-of-range input, never exceeds `total` (2) or goes negative, and stays monotonic when one file advances while the other holds and when the two files' reports interleave.
