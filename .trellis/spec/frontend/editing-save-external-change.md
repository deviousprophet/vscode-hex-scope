...
External edits to `.hexscope/` profile/pool files do **not** follow the hex-file external-change contract below. They are hosted silently: the profile watcher (`attachProfileWatcher`) → per-slot debounced reload (`JsonStore.scheduleReload`) → re-read + re-normalize → re-broadcast to the webview (`structsExternalChange` / `perFileDataChange` — the latter carries the profile's `activeChecks` slice / `profilesState` for registry changes). No prompt, no lock, no conflict dialog, no repair/discard UI anywhere; the self-write horizon ignores host-issued writes so save/self-heal never re-triggers the watcher. The watcher is attached on **every** session open (bound or not) so external registry edits refresh the open dropdown; for an out-of-workspace open with no `.hexscope/` yet it stays inert (the `RelativePattern` matches nothing until files exist), so no false external-change events can originate from a bare open.
...
- Document watcher events are gated by `createExternalChangeGate` (`src/core/documentExternalChange.ts`): an event is only a real external change when the initial load has completed (`parseResult !== null`), the re-read `newRaw` differs from the loaded `raw`, and the write is not inside the self-write horizon. A copy/create event for the open file and duplicate watcher emissions for one write therefore post no banner.
