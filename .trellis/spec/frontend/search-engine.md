...
Applies to `core/search.ts` (deep module) and the webview glue `src/webview/search/searchEngine.ts`. The search bar UI unit is owned by [SearchBar Component Code-Spec](./components/component-search-bar.md); selection/inspector/copy contracts by [Selection, Inspector, and Byte Tools Code-Spec](./selection-inspect-copy.md).
...
- Search UI state (mode/endianness/query) is owned by the `SearchBar` component; the engine glue takes explicit params (`runSearch(query, mode, endianness, trigger)`) and never reads `S.searchMode`/`S.searchEndianness`/the input DOM for its decision. `S.searchMode`/`S.searchEndianness` are written by the host in the `onSearch` handler. Match state (`S.matchAddrs`/`S.matchIdx`) is written by the glue and read by renderers.
- **Match-highlight truth = executed search:** the glue stores the executed needle byte-span in `S.searchMatchSpan` when a search starts, and clears it (`0`) on `clearSearch`/empty-query. The memory grid reads only `S.searchMatchSpan` for highlight width — never the live input or `S.searchMode` (no `NEEDLE_LEN_BY_MODE` re-derivation).
- **Divergence invalidation:** `invalidateSearchIfDiverged(query, mode, endianness)` cancels a diverged in-flight search (`engine.clear()` + drop running flag) or clears the completed match set + resets the count when the visible query/mode/endian diverges from the running (`_activeSearchKey`) or completed (`_lastCompletedSearchKey`) search key. Emptying the query counts as diverged. It no-ops when nothing is running and no matches exist. Called by the host on `SearchBar.onQueryChanged` (UI-only changes); also resets `_lastCompletedSearchKey` so re-running the same query re-searches.
