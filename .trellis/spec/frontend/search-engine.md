...
- **Divergence invalidation:** `invalidateSearchIfDiverged(query, mode, endianness)` clears the completed match set + resets the count when the visible query/mode/endian diverges from the executed search key (`searchKeyFor`); it no-ops while a search is running or when no matches exist. Called by the host on `SearchBar.onQueryChanged` (UI-only changes).
