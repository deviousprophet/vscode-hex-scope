...
export function decodePairKey(key: string): { aPath: string; bPath: string } {
    ...
    const [ap, bp] = pairCanonical(pair[0], pair[1]);
...
function parsePairKey(key: string): [string, string] {
    ...
    if (isStringPair(pair)) { return pair; }
    throw new Error('invalid pair key');
}

function isStringPair(pair: unknown): pair is [string, string] {
    return Array.isArray(pair) && pair.length === 2 && pair.every(p => typeof p === 'string');
}
