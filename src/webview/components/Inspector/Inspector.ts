...
import { actionBtnsHtml, esc, fmtB, formatDecimal, formatHex, wireActionBtns } from '../../utils';
...
function labelItemHtml(label: SegmentLabel, index: number, count: number): string {
...
function labelFormHtml(editing: SegmentLabel | undefined, swatchHtml: string, defaultStart: string, defaultRange: string): string {
    const mode = editing
...
function labelsBadgeHtml(count: number): string {
    return count > 0 ? `<span class="sb-badge">${count}</span>` : '';
}

function defaultLabelColor(editing: SegmentLabel | undefined, fallback: string): string {
    return editing ? editing.color : fallback;
}

function mergeLabel(labels: SegmentLabel[], editId: string | undefined, label: SegmentLabel): SegmentLabel[] {
...
function endAddressOrEmpty(start: number, length: number): string {
...
function isOutsideMappedData(segments: SerializedSegment[], startAddress: number, endAddress: number): boolean {
